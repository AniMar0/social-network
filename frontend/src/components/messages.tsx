        ...
        if (newMessage.trim()) {
            ...
            const message: Message = {
                ...
                replyTo: replyingTo ? {
                    ...
                    senderName: replyingTo.isOwn ? "You" : selectedChat?.name || "Unknown"
                } : undefined
            };
                    ...
                    <div className="space-y-2">
                        {filteredChats.map((chat) => (
                            <div
                                ...
                                className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors ${selectedChat?.id === chat.id ? "bg-accent text-accent-foreground" : "hover:bg-muted/50"
                                    }`}
                            >
                                <div className="relative">
                                    <Avatar className="h-10 w-10">
                                        <AvatarImage src={chat.avatar || "https://i.imgur.com/aSlIJks.png"} alt={chat.name} />
            ...
            {selectedChat ? (
                <div className="flex-1 flex flex-col">
                        ...
                        <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <Avatar className="h-8 w-8">
                                    <AvatarImage src={selectedChat.avatar || "https://i.imgur.com/aSlIJks.png"} alt={selectedChat.name} />
                                    <AvatarFallback className="bg-muted text-foreground">
                    ...
                    <div className="p-6 border-b border-border bg-card text-center">
                        ...
                        <p className="text-sm text-muted-foreground mb-2">{selectedChat.username}</p>
                        <p className="text-sm text-foreground mb-2">{userProfile.age}, {userProfile.profession}.</p>
                        <p className="text-sm text-muted-foreground mb-3">{userProfile.bio}</p>
                        <div className="text-xs text-muted-foreground space-y-1">
                            <p>Joined {userProfile.joinDate} · {userProfile.followers}</p>
                            <p>{userProfile.followedBy}</p>
                        </div>
                    </div>
                                ...
                                <div key={message.id} className={`flex ${message.isOwn ? "justify-end" : "justify-start"}`}>
                                    <div className={`max-w-xs lg:max-w-md ${message.isOwn ? "order-2" : "order-1"}`}>
                                        <ContextMenu>
                                            <ContextMenuTrigger>
                                                <div className={`${message.isOwn ? "ml-auto" : "mr-auto"} max-w-xs lg:max-w-md`}>
                                                    {/* Reply indicator */}
                                                    {message.replyTo && (
                                                        <div className={`mb-1 px-3 py-1 rounded-t-sm text-xs border-l-2 ${
                                                            message.isOwn 
                                                    ...
                                                    {message.type === "emoji" ? (
                                                        <div className={`text-6xl cursor-pointer ${message.replyTo ? 'rounded-b-sm' : 'rounded-sm'}`}>
                                                            {message.content}
                                                        </div>
                                                    ) : message.type === "gif" ? (
                                                        <div className={`overflow-hidden max-w-xs cursor-pointer ${message.replyTo ? 'rounded-b-sm' : 'rounded-sm'}`}>
                                                            <img 
                                                                src={message.content} 
                                                                alt="GIF" 
                                                        ...
                                                        <div
                                                            className={`px-4 py-2 cursor-pointer ${message.replyTo ? 'rounded-b-sm' : 'rounded-sm'} ${
                                                                message.isOwn 
                                        ...
                                        <div className={`flex items-center gap-2 mt-1 ${message.isOwn ? "justify-end" : "justify-start"}`}>
                                            ...
                                            {message.isRead && <span className="text-xs text-muted-foreground">· Seen</span>}
                                        </div>
                                    </div>
                                </div>
                            ))}
                            ...
                            <div className="flex items-center justify-between">
                                <div className="flex-1">
                                    <p className="text-xs text-muted-foreground mb-1">
                                        Replying to {replyingTo.isOwn ? "yourself" : selectedChat?.name}
                                    </p>
                                    <p className="text-sm text-foreground truncate max-w-md">
                                        {replyingTo.type === "emoji" ? replyingTo.content : 
